...
switch ($solicitud) {
    ...
    case "ConsultasVentas":
        $parametros = ['fechaParticular', 'email', 'precio1', 'precio2', 'segundaFecha'];
        $tipos = ['string', 'email', 'float', 'float'];

        if (isset($_GET['fechaParticular']) && validarParametro($_GET['fechaParticular'], $tipos[0])) {
            $fecha = $_GET['fechaParticular'];
        }
        else {
            $fecha = date("Y-m-d");
        }
        ...
        try {
            consultasVentas::obtenerConsultaVentas("Jsons/tienda.json", "Jsons/ventas.json", $fecha, $_GET['email'], $_GET['precio1'], $_GET['precio2'], $mensaje);
            //tiendaAlta::darAltaProducto("Jsons/tienda.json", $unProducto, $_FILES['imagen'], $mensaje);
            echo $mensaje;
        } catch (Exception $e) {
            enviarRespuesta(500, "Error: " . $e->getMessage());
